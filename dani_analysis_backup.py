...
#notes to self
#see lines ~490 in analysis-clean.py?
...
def execute():
    ...
    #RPKM = f'{d}/data/daniela/Projects/sujin/outputs/{outdir}RPKM_calculations.txt'
    RPKM_input = '/data/daniela/Projects/sujin/outputs/RPKM_calculations_input.txt'
    # PLOT_DIR   = f'{d}/data/daniela/Projects/sujin/outputs/{outdir}plots/'
    PLOT_DIR = BASE_DIR + 'temp_graphs5/'
    ...
    for r in REGIONS:
        for h in HIST:
            ...
            for i,s in enumerate(tqdm(STRAINS)):
                ...
                fig = smoothplot(bg, i+1, strain_map, r, fig=fig, 
...
#     return ( (min(x_mins), max(x_maxs)), (min(y_mins), max(y_maxs)) )
    return ( (min(x_mins), max(x_maxs)), (1, max(y_maxs)*1.1) )

def smoothplot(bin_group, i, strain_map, region, x_metric='id', y_metric='subtracted',
    ...
    plt.ylabel(y_label)
    if region == 'cen':
        plt.yticks(np.arange(min(y_lim), max(y_lim), 0.5))
    else:
